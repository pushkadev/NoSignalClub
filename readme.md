# WhatsApp SMS Forwarder (Android)

Android-приложение для автоматической пересылки **входящих сообщений WhatsApp** на указанный номер телефона в виде **SMS**.
...
- 📥 Перехват входящих сообщений WhatsApp через системные уведомления
...
Пользователь ходит с кнопочным телефоном, но хочет оставаться в курсе сообщений в WhatsApp.  
...
Приложение принимает сообщения WhatsApp при наличии интернета и пересылает их на номер, который находится вне зоны 4G.
...
Приложение позволяет получать уведомления о сообщениях WhatsApp в виде обычных SMS.

---

## 🏗 Архитектура

- Используется `NotificationListenerService` для отслеживания уведомлений WhatsApp
